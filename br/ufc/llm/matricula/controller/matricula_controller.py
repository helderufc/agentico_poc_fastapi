...
from br.ufc.llm.matricula.dto.matricula_dto import (
    MatriculaRequest, MatriculaResponse, ListaMatriculasResponse,
    ListaCatalogoCursosResponse, CursoResumidoResponse,
    ListaModulosResponse, ListaAulasResponse, AulaCompletaResponse,
    ProvaSemGabaritoResponse, ResultadoProvaResponse, RespostaItemRequest,
)
...
from br.ufc.llm.matricula.exception.matricula_exception import (
    MatriculaJaExisteException, MatriculaNaoEncontradaException,
    CursoNaoPublicadoException, AcessoNegadoMatriculaException,
    PerfilInvalidoException, DadosMatriculaObrigatoriosException,
    RespostaJaExisteException, RespostasIncompletasException,
...
async def listar_catalogo(
    ...
    resultado = await service.listar_catalogo(q)
...
async def obter_curso_catalogo(
    ...
    try:
        service = MatriculaService(session)
        curso = await service.obter_curso_publicado(curso_id)
...
async def matricular(
    ...
    try:
        ...
        service = MatriculaService(session)
        matricula = await service.matricular(aluno_id, perfil, requisicao)
...
async def listar_minhas_matriculas(
    ...
    aluno_id = int(payload.get("sub"))
    service = MatriculaService(session)
    resultado = await service.listar_minhas_matriculas(aluno_id)
...
async def listar_modulos(
    ...
    try:
        aluno_id = int(payload.get("sub"))
        service = MatriculaService(session)
        resultado = await service.listar_modulos(aluno_id, curso_id)
...
@router.get("/matriculas/{curso_id}/modulos/{modulo_id}/aulas", response_model=RespostaPadrao[ListaAulasResponse])
async def listar_aulas(
    ...
    try:
        aluno_id = int(payload.get("sub"))
        service = MatriculaService(session)
        resultado = await service.listar_aulas(aluno_id, curso_id, modulo_id)
        return RespostaPadrao(data=resultado, message="Aulas listadas com sucesso", status=200)
    except AcessoNegadoMatriculaException as e:
        raise HTTPException(status_code=403, detail=e.message)


@router.get("/matriculas/{curso_id}/modulos/{modulo_id}/aulas/{aula_id}", response_model=RespostaPadrao[AulaCompletaResponse])
async def obter_aula(
    curso_id: int, modulo_id: int, aula_id: int,
    payload: dict = Depends(_obter_payload),
    session: AsyncSession = Depends(get_db)
):
    try:
        aluno_id = int(payload.get("sub"))
        service = MatriculaService(session)
        aula = await service.obter_aula(aluno_id, curso_id, modulo_id, aula_id)
...
@router.get("/matriculas/{curso_id}/modulos/{modulo_id}/prova", response_model=RespostaPadrao[ProvaSemGabaritoResponse])
async def obter_prova(
    curso_id: int, modulo_id: int,
    payload: dict = Depends(_obter_payload),
    session: AsyncSession = Depends(get_db)
):
    try:
        aluno_id = int(payload.get("sub"))
        service = MatriculaService(session)
        prova = await service.obter_prova(aluno_id, curso_id, modulo_id)
...
async def responder_prova(
    ...
    try:
        aluno_id = int(payload.get("sub"))
        service = MatriculaService(session)
        await service.responder_prova(aluno_id, prova_id, respostas)
...
async def obter_resultado(
    ...
    try:
        aluno_id = int(payload.get("sub"))
        service = MatriculaService(session)
        resultado = await service.obter_resultado(aluno_id, prova_id)
